...
def process_files(pdf_file_path: str, collection: chromadb.Collection) -> None:
    ...
    for d in to_process_list:
        ...
        add_documents(doc_list=doc_list, model=os.getenv(
            "EMBEDDING_MODEL"), collection=collection)
...
Contextual data: 
{context}
In your answer stay as close as possible to the wording of the contextu al data, if possible quote it. If you are unable to answer the question by the provided contextual data, reply 'I dont know'.  The contextual data stems from a scientific research paper called {doc_title}.
 """),
            # Return the contextual data before your response.
    ...
    with st.sidebar:
        ...
        document_title = st.selectbox("Select the document you would like to chat with", get_pdf_titles(
            collection=collection), index=None, placeholder="Select a document to chat with")
        if st.button('Process newly added files'):
            with st.spinner("Processing"):
                process_files(pdf_file_path=pdf_file_path,
    ...
    if prompt := st.chat_input('Ask your question about this paper'):
        context = retriever(
            prompt=prompt, doc_title=document_title, collection=collection, n_results=3)
        # add latest message to history in format {role, content}
        response = chain.invoke({"input": prompt, "context": context,
                                "doc_title": document_title, "chat_history": st.session_state["chat_history"]})
        print(context)
